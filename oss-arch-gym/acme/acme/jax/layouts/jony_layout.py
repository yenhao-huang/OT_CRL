...
from acme.agents import agent, jony_agent
...
class DRAMLayout(jony_agent.Agent):
  ...
  def __init__(
    ...
    oldreplay_client = None

    if ld_rp_ckpt_path:
      replay_checkpointer = reverb.platform.checkpointers_lib.DefaultCheckpointer(path=ld_rp_ckpt_path)
    else:
      replay_checkpointer = reverb.platform.checkpointers_lib.TempDirCheckpointer()
    
    self.replay_checkpointer = replay_checkpointer
    replay_server = reverb.Server(replay_tables, port=None, checkpointer=replay_checkpointer)
    replay_client = reverb.Client(f'localhost:{replay_server.port}')
    print(f"Replay buffer size = {list(replay_client.server_info().values())[0]}")

    # Save checkpoint, TODO: don't write on this path
