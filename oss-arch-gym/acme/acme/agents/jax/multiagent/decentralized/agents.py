...
from acme.jax.layouts import jony_layout
...
class DecentralizedMultiAgent(jony_layout.DRAMLayout):
  ...
  def __init__(
    ...
    if logger_factory == None:
        ...
        learner_logger_fns = decentralized_factories.jony_logger_factory(
            agent_types=agent_types,
            base_label='learner',
            steps_key='learner_steps',
    ...
    super().__init__(
        ...
        ld_rp_ckpt_path=config.ld_rp_ckpt_path,
    )
    # pytype: enable=wrong-arg-types
...
def init_decentralized_multiagent(
  ...
  config = decentralized_config.DecentralizedMultiagentConfig(
      batch_size=batch_size, sub_agent_configs=configs, ld_rp_ckpt_path=ld_rp_ckpt_path,)
