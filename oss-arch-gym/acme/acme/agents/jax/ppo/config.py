...
class PPOConfig:
  ...
  '''
  [JONY]
  batch_size : how much sample per update
  metrics_logging_period : how many update per logging to learner log
  '''
  unroll_length: int = 8
  ...
  #learning_rate: Union[float, Callable[[int], float]] = 2e-5 # SINGLE AGENT
  adam_epsilon: float = 1e-7
  entropy_cost: float = 3e-4
  bc_cost: float = 1.0
  end_explore_updateidx: int = 1000000
  #entropy_cost: float = 0.1 # SINGLE AGENT
  value_cost: float = 1.
  max_gradient_norm: float = 0.5
  variable_update_period: int = 1
  log_global_norm_metrics: bool = False
  #metrics_logging_period: int = 100
  metrics_logging_period: int = 1
