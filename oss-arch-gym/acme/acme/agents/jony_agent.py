...
class Agent(core.Actor, core.VariableSource):
  ...
  def __init__(self, actor: core.Actor, learner: core.Learner,
    ...
    self.table_info = list(replay_client.server_info().values())
  ...
  '''
  def _has_data_for_training(self):
    if ((self._num_observations-1) % self.batch_size == 0) and ((self._num_observations-1) != 0):
      return True
    else:
      return False
  '''
  # measuer learning time  
  def update(self):
    ...
    if self._iterator:
      ...
      update_actor = False
      if self._has_data_for_training():
        ...
        #self._batch_size_upper_bounds = [
        #    math.ceil(t.rate_limiter_info.sample_stats.completed /
        #              (total_batches + 1)) for t in self.table_info
        #]

        print(f"Replay buffer size = {self.table_info[0].rate_limiter_info.sample_stats.completed}")
        update_actor = True
