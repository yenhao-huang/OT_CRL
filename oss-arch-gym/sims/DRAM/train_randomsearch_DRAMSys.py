...
flags.DEFINE_string('workload', 'swaptions', 'Which DRAMSys workload to run?')
flags.DEFINE_string('cost_model', 'simulator', 'simulator/buffer/proxy_model')
flags.DEFINE_integer('num_steps', 100, 'Number of training steps.')
flags.DEFINE_integer('group_idx', None, 'Index for configuration')
...
def main(_):
    ...
    update_thread()
    env = dramsys_wrapper.make_dramsys_env(FLAGS.workload, cost_model=FLAGS.cost_model)
    ...
    '''
    # 0:opened; 1: closed
    problem.search_space.select_root().add_discrete_param(name='pagepolicy', feasible_values=[0, 1])
    # 0: map1; 1: map2 ~ 4: map5
    problem.search_space.select_root().add_discrete_param(name='mapping', feasible_values=[0, 1, 2, 3, 4])
    problem.search_space.select_root().add_int_param(name='rcd', min_value = 12, max_value = 16)
    problem.search_space.select_root().add_int_param(name='rp', min_value = 12, max_value = 16)
    problem.search_space.select_root().add_int_param(name='ras', min_value = 19, max_value = 39)
    problem.search_space.select_root().add_int_param(name='rrd', min_value = 3, max_value = 6)
    problem.search_space.select_root().add_discrete_param(name='refi', feasible_values=list(range(4680, 468000, 46800)))
    ...
    '''
     # experiment name 
    exp_name = "_num_steps_" + str(FLAGS.num_steps) + "_num_episodes_" + str(FLAGS.num_episodes) + "_" + str(FLAGS.group_idx)

    # append logs to base path
    log_path = os.path.join(FLAGS.summary_dir, 'random_search_logs', FLAGS.reward_formulation, FLAGS.workload, exp_name)

    # get the current working directory and append the exp name
    traject_dir = os.path.join(FLAGS.summary_dir, FLAGS.traject_dir, FLAGS.reward_formulation, FLAGS.workload, exp_name)
    ...
    for suggestion in suggestions:
        ...
        mapping = str(suggestion.parameters['mapping'])
        ...
        refi = str(suggestion.parameters['refi'])

        vizier_actions = [int(pagepolicy), int(mapping), int(rcd), int(rp), int(ras), int(rrd), int(refi)]
        print(vizier_actions)
        
        action = np.asarray(vizier_actions)
        action_dict = dram_helper.action_decoder_random(action)
