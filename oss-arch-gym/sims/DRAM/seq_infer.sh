...
threadidx=1385
ckpt_threadidx=20000

cp inference_marl.sh shell_log/rl/thread$threadidx.sh

mkdir -p logs/Thread_${threadidx}

for idx in "${!workloadset[@]}"; do
    workload=${workloadset[$idx]}

    if [[ $idx -eq $((${#workloadset[@]} - 1)) ]]; then
        next_workload=${workloadset[0]}
    else
        next_workload=${workloadset[$((idx + 1))]}
    fi
    CHECKPOINT_DIR="/home/user/acme/checkpoint/singleworkload/${workload}"

    # w learning
    python3 train_multiagent.py --only_acting --eval_episodes 1 \
    --workload $next_workload --threadidx $threadidx \
    --checkpoint_dir $CHECKPOINT_DIR/checkpoints/learner --load_checkpoint \
    --expbuf_threadidx $ckpt_threadidx \
    --summarydir logs/Thread_${threadidx}/${next_workload}/ || exit 1
