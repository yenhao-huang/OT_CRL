...
class helpers():
    ...
    def action_mapper(self, action, param):
        ...
        """
        num_bins = len(param)
        action_bin = 2.002/num_bins
        
        # create boundries for each bin
        boundries = np.arange(-1.001, 1.001, action_bin)
    ...
    def action_decoder_random(self, act_encoded):
        """
        ...
        act_decoded["pagepolicy"] = pagepolicy[act_encoded[0]]
        act_decoded["addressmapping"] = addressmapping[act_encoded[1]]
        act_decoded["rcd"]  =  act_encoded[2]
        act_decoded["rp"]  =  act_encoded[3]
        act_decoded["ras"]  =  act_encoded[4]
        act_decoded["rrd"]  =  act_encoded[5]
        act_decoded["refi"] = act_encoded[6]
    ...
    def action_decoder_rl(self, act_encoded, rl_form):
        ...
        rrd_mapper = dict([(i, config) for i, config in enumerate(range(3, 7))])
        refi_mapper = dict([(i, config) for i, config in enumerate(range(4680, 468000, 46800))])


        if(rl_form == 'sa' or rl_form == 'macme_continuous'):
            act_decoded["pagepolicy"] = pagepolicy[self.action_mapper(act_encoded[0], pagepolicy)]
            act_decoded["addressmapping"] = addressmapping[self.action_mapper(act_encoded[1], addressmapping)]
            ...
            act_decoded["refi"] = refi_mapper[self.action_mapper(act_encoded[6], refi_mapper)]
    ...
    def action_decoder_marl(self, act_encoded, rl_form):
        """
        Decode the action space for the RL agent
        """
        print("[Action Encoded]", act_encoded)
        ...
        rrd_mapper = dict([(i, config) for i, config in enumerate(range(3, 7))])
        # TODO
        #refi_mapper = dict([(i, config) for i, config in enumerate(range(4680, 18720, 1560))])
        refi_mapper = dict([(i, config) for i, config in enumerate(range(4680, 472680, 46800))])

        act_decoded["pagepolicy"] = pagepolicy[act_encoded[0]]
        act_decoded["addressmapping"] = addressmapping[act_encoded[1]]
        act_decoded["rcd"]  =  rcd_mapper[act_encoded[2]]
        act_decoded["rp"]  =  rp_mapper[act_encoded[3]]
        act_decoded["ras"]  =  ras_mapper[act_encoded[4]]
        act_decoded["rrd"]  =  rrd_mapper[act_encoded[5]]
        act_decoded["refi"] = refi_mapper[act_encoded[6]]
    ...
    def action_decoder_ga(self, act_encoded):
        ...
        act_decoded["refi"]  =  int(float(act_encoded[8]))
    ...
    def convert_to_model_format(self, err_factor):
        refi = err_factor[0]
        rcd = err_factor[1] * self.tCK
        rp = err_factor[2] * self.tCK
        ras = err_factor[3] *self.tCK
        return refi, rcd, rp, ras

        
    def get_err_rate(self, action):
        refresh_interval, rcd, rp, ras = self.convert_to_model_format([action["refi"], action["rcd"], action["rp"], action["ras"]])
        #refresh_interval, rcd, rp, ras = self.convert_to_model_format([374400, 5, 5, 19])
        #print(f"Refresh Interval: {refresh_interval}, RCD: {rcd}, RP: {rp}, RAS: {ras}")
        error_rate = self.error_model(refresh_interval, rcd, rp, ras)
        return error_rate
    ...
    def read_modify_write_dramsys(self, action):
        ...
        try:
            with open (mem_spec_file, "r") as JsonFile:
                ...
                data['memspec']['memtimingspec']['REFI'] = action['refi']
    ...
    def read_modify_write_ramulator_random(self, action):
        print("[envHelpers][Action]", action)
        op_success = False
    ...
    def read_modify_write_ramulator(self, action, thread_idx):
        print("[envHelpers][Action]", action)
        op_success = False
        #mem_spec_file = f"/home/user/ramulator/lib/tuner_ddr4/config.txt"
        mem_spec_file = f"/home/user/ramulator/lib/tuner_ddr4_multithread/config/config_{thread_idx}.txt"
