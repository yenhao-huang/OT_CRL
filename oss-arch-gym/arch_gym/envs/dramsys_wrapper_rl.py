...
import numpy as np
import tree
from DRAMEnv_RL import DRAMEnv_V6
DRAMEnv = DRAMEnv_V6
...
class DRAMSysEnvWrapper(dm_env.Environment):
  ...
  '''
  def step_first(self) -> dm_env.TimeStep:

    if(self.env_wrapper_sel=='macme' or self.env_wrapper_sel=='macme_continuous'):
      agents_action = [0] * 7
      observation, reward, done, info = self._environment.step_first(agents_action)
    else:
      action = [0] * 7
      observation, reward, done, info = self._environment.step_first(action)
...
def make_dramsys_env(workload,
                    ...
                    expertdata_path = None) -> dm_env.Environment:
  ...
  environment = DRAMSysEnvWrapper(
    DRAMEnv(
      ...
      is_update_buffer = is_update_buffer,
