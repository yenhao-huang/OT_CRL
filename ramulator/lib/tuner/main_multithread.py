...
def change_config(config_pair, workload_pair):
    ...
    i_path = os.path.join(HOME_DIR, 'configs', 'ours', f'DDR3-sample.cfg')
    o_path = os.path.join(HOME_DIR, 'configs', 'ours', f'DDR3-config_{workload_idx}.cfg')
    map_config_path = {
    ...
    if config[0] in map_config_path:
        map_config = os.path.join(HOME_DIR, 'mappings', 'ours', map_config_path[config[0]])
    else:
...
def execute_sim(config_pair, workload_pair):
    ...
    config_path = os.path.join(HOME_DIR, 'configs', 'ours', f'DDR3-config_{workload_idx}.cfg')
