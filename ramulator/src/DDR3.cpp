...
std::vector<int> read_config_ddr3(const std::string& filename) {
    ...
    if (values.size() != 7) {
        throw std::runtime_error("Incorrect number of configuration values");
    }
...
void DDR3::init_timing()
{
    ...
    /*
    std::cout << "CL: " << s.nCL << " BL: " << s.nBL  << " RCD: " << s.nRCD << " RAS: " << s.nRAS << " WR: " << s.nWR << " RP: " << s.nRP << " REFI: " << s.nREFI << std::endl;

    std::vector<int> config = read_config_ddr3("/home/user/ramulator/lib/option_timing.txt");
    std::cout << "CL: " << config[5] << " BL: " << config[6]  << " RCD: " << config[0] << " RAS: " << config[1] << " WR: " << config[2] << " RP: " << config[3] << " REFI: " << config[4] << std::endl;
