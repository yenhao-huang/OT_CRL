...
DDR4::DDR4(Org org, Speed speed, string pre_timing)
    ...
    read_latency(speed_entry.nCL + speed_entry.nBL)
{
    ...
    init_rowopen();
    init_lambda();
    init_timing(pre_timing);
}

DDR4::DDR4(const string& org_str, const string& speed_str, string pre_timing) :
    DDR4(org_map[org_str], speed_map[speed_str], pre_timing) 
{
}
...
void DDR4::init_timing(std::string pre_timing)
{
    SpeedEntry& s = speed_entry;
    vector<TimingEntry> *t;

    std::vector<int> config;
    std::istringstream iss(pre_timing);
    std::string token;
    while (std::getline(iss, token, ',')) {
        config.push_back(std::stoi(token));
    }
    /*
    std::cout << " RCD: " << s.nRCD << " RP: " << s.nRP << " RAS: " << s.nRAS << "RRD" << s.nRRDL <<  " REFI: " << s.nREFI << std::endl;
    std::cout << " RCD: " << config[0] << " RP: " << config[1] << " RAS: " << config[2] << "RRD" << config[3] << " REFI: " << config[4] << std::endl;
    */
    s.nRCD = config[0];
    s.nRP = config[1];
    s.nRAS = config[2];
